...
> "This is a mitigation, not a complete answer... 원장이 비어있다는 게
> '아무 작업도 없다'와 같은 뜻이 아니다. 턴의 result 프레임 *직전*에
> 작업이 끝나버리면, 그 시점엔 원장이 비어있지만 그 작업의 완료가 부모를
> 깨워 후속 턴을 시작시킬 수도 있다. 이 간극은 어떤 원장으로도 못 닫는다
> — 엔진 쪽에서 명시적인 '턴 경계' 신호를 보내주지 않는 한."
...
> 온다. subtype만 보고 에러 메시지를 만들면 "성공했다는 에러가
> 발생했습니다" 같은 자기모순적인 메시지가 나온다.
